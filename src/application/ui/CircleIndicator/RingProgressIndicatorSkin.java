 ...
 */
package application.ui.CircleIndicator;
...
public class RingProgressIndicatorSkin implements Skin<RingProgressIndicator> {
	...
	public RingProgressIndicatorSkin(final RingProgressIndicator indicator) {
		...
		this.indicator.labelProperty().addListener((o, oldStr, newStr) -> {
			setProgressLabel(indicator.getProgress(), newStr);

		});

		this.indicator.progressProperty().addListener((o, oldVal, newVal) -> {
			if (newVal.intValue() >= 0) {
				setProgressLabel(newVal.intValue(), indicator.getLabel());
		...
		initLabel(indicator.getProgress(), indicator.getLabel());
	...
	private void setProgressLabel(double value, String label) {
		if (value >= 0) {
			percentLabel.setText(label);
		}
	}
	...
	private void initLabel(double value, String label) {
		setProgressLabel(value, label);
